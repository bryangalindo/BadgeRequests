...
async def create_application(application: Application):
    ...
    application_dict = dict(
        PartitionKey=application.PartitionKey, RowKey=application.RowKey)
...
async def delete_application(application_id: str, admin=Depends(get_current_admin)):
    ...
    applications_table = Table('applications')
    application_data_generator = applications_table.query(
        "RowKey", application_id)
    application_data = list(application_data_generator)[0]
    applications_table.delete(
        partition_key=application_data.PartitionKey, row_key=application_data.RowKey)
    return {"Details": "Application deleted"}


@router.get("/")
async def get_all_applications(admin=Depends(get_current_admin)):
    applications_table = Table('applications')
